...
def home():
    if current_user.is_authenticated:
        ...
        cm = CompanyManager.query.filter(
            CompanyManager.id == current_user.get_id()).first()
        ...
        basic_info = BasicInfo.query.filter(
            BasicInfo.id == current_user.get_id()).first()
        future = 0
        nowa = 0
...
def project_profile(id):
    if current_user.is_authenticated:
        ...
        if form.validate_on_submit():
            ...
            info = BasicInfo.query.filter(
                BasicInfo.id == form.pmid1.data).first()
            if info:
                pm = ProjectManager.query.filter(
                    ProjectManager.id == info.id).first()
                if pm:
                    project.PMID.append(pm)
                else:
                    ca = Candidate.query.filter(
                        Candidate.id == info.id).first()
                    if ca:
                        info.UserType = 1
                        info.candidate = None
...
def delete(id):
    if current_user.is_authenticated:
        ...
        project = Project.query.filter(Project.PID == id).first()
        pm = ProjectManager.query.filter(
            ProjectManager.PID == Project.PMID).all()
        for item in pm:
            # 恢复candidate身份
            somthin = None
